...
La figura \ref{fig:fr_clusters} introduce las palabras con mayor frecuencia de los tres \textit{clusters} considerados. El primer \textit{cluster} (0) presenta algunas palabras relevantes al inicio de la pandemia como lo son \textit{confinement}, \textit{vaccin} y \textit{monde} entre otras. El segundo \textit{cluster} (1) introduce otro conjunto de palabras relevantes como \textit{deconfinement}, \textit{vaccin}, \textit{pandemie} y \textit{masque}. Finalmente, el tercer \textit{cluster} (2) presenta palabras como \textit{variant} y \textit{vaccin}. 
...
La figura \ref{fig:fr_time} presenta la distribución de los grupos de palabras a través del tiempo. Si bien hay una distinción entre los conjuntos y su temporalidad, esta no es tan clara como se ha logrado identificar en los otros idiomas. No obstante, algunos grupos con palabras únicas presentan repuntes que se pueden relacionar con los temas de interés del momento. Por ejemplo, el tercer \textit{cluster} (2) presenta un repunte en el mes de febrero de 2021, época en la cual la variante india fue identificada. Otro aspecto que se puede identificar es el repunte del grupo (1) en el mes de marzo de 2020. Un término de dicho grupo es \textit{pandemie} que tuvo popularidad cuando la OMS declaró la pandemia de COVID-19 el 11 de marzo de 2020.
